...
O \textit{PlatformIO} é uma ferramenta profissional de plataforma cruzada, ou seja, disponível em diversos \textit{Softwares} de desenvolvimento, e de estrutura múltipla voltada para desenvolvedores de software embarcado.

A ferramenta fornece um ambiente de desenvolvimento integrado moderno, oferecendo suporte a diversos \textit{kits} de desenvolvimento de \textit{software} (\textit{SDKs}) ou \textit{Frameworks} e inclui depuração sofisticada (\textit{Debug Mode}), testes de unidade (\textit{Unit Test}), análise automatizada de código (\textit{Static Code Analysis}) e gerenciamento remoto (\textit{Remote Development}). É arquitetado para maximizar a flexibilidade e escolha dos desenvolvedores, que podem usar editores gráficos e/ou de linha de comando (\textit{PlatformIO Core}(\textit{CLI})).

Uma extensão PlatformIO IDE nativa para o editor Microsoft VSCode é a extensão mais avaliada / revisada, com mais de 800 avaliações cinco estrelas em todo o Microsoft Marketplace. Ele também foi instalado por mais de 750.000 desenvolvedores exclusivos em todo o mundo.\textbf{}


criada em 2005, não abrange somente as placas microcontroladas mas também agrega-se de uma Interface de Desenvolvimento, ou seja uma IDE, que possui um compilador \textit{C++}. A utilização dessa IDE se tornou tão difundida atualmente que grandes empresas de tecnologia têm buscado tornar ou fabricar microcontroladores completamente compatíveis com a interface.  
Com esta interface, será possível a programação do microcontrolador \textit{ESP8266} de maneira facilitada e sem a necessidade do aprendizado de uma nova linguagem. Todas as funcionalidades, como rotinas de conexão \textit{HTTP} e/ou \textit{MQTT} através do \textit{Wi-Fi}, Interrupção, Timers, Modos de Energia são completamente acessíveis. 
